"""Scraper de producción DFAI: recorre el listado de resoluciones de la
DFAI, filtra por sector (env var SECTORES, default hidrocarburos/industria)
y envía las resoluciones en alcance a n8n vía webhook, en lotes (inserción
simple en `resoluciones_dfai`; el parseo por secciones y las referencias
cruzadas se hacen después, igual que en el flujo TFA).

Copia estructural de produccion.py -- mismo patrón de descarga, reintentos
y envío por lotes -- apuntando al listado y webhook de la DFAI en vez del
TFA. Ver CLAUDE.md "Fuentes de datos": la DFAI capta los casos firmes que
no llegan a apelación (subsanación voluntaria, reconocimiento de
responsabilidad), que el corpus TFA-only no tiene.

Los regex de extracción (listado, PDF, encabezado) se heredan sin
verificar contra la estructura real de la página de la DFAI -- gob.pe
bloqueó el acceso directo durante el desarrollo de este scraper. La
primera corrida real (vía GitHub Actions, con headers de navegador) es la
que confirma si matchean; si no, hay que ajustar los patrones con un
ejemplo real a la vista.
...
N8N_WEBHOOK_URL = os.environ.get("N8N_WEBHOOK_URL") or (
    "https://n8n-digitalocean.ai-salva.com/webhook/pas360-resolucion-dfai"
)
...
# comas), default = mismo alcance que TFA (hidrocarburos/industria). Ver
...
def extraer_numero_de_url(pdf_url: str) -> str | None:
    ...
    extraído del PDF (visto en resoluciones antiguas del TFA con encabezado
    fuera de formato). El nombre de archivo en la URL del CDN suele ser
    literalmente "RESOLUCIÓN N° XXX-YYYY-OEFA/...pdf", así que el mismo
    regex aplicado al path decodificado también sirve aquí."""
    match = RESOLUCION_RE.search(unquote(pdf_url))
    return match.group(1) if match else None


def extract_text(pdf_bytes: bytes) -> str:
    with open("/tmp/r_dfai.pdf", "wb") as f:
        f.write(pdf_bytes)
    paginas = []
    with pdfplumber.open("/tmp/r_dfai.pdf") as pdf:
...
def procesar_resolucion(detalle_href: str):
    ...
    # "Unexpected EOF") -- visto en el scraper TFA, sobre todo en
    # resoluciones antiguas. Un segundo intento normalmente basta.
    try:
...
def main() -> None:
    ...
    if hrefs_extra:
        ...
        despachar_lote()
        with open("resultados_dfai.json", "w", encoding="utf-8") as f:
